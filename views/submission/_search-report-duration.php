...
if ($currentRole['role_id'] == Role::PRESIDENT || $currentRole['role_id'] == Role::COPRESIDENT) {
    ...
    if ($status == Submission::STATUS_DOC_REJECTED || ($status == Submission::STATUS_SUBMITTED && $typeGroup == app\models\SubmissionTypeGroup::GROUP_NEW)) {
        $data = Submission::getStatusLabelsNopanel();
    } elseif ($status == Submission::STATUS_SUBMITTED && $typeGroup == app\models\SubmissionTypeGroup::GROUP_CONT) {
...
<div class="submission-search">
    ...
    $form = ActiveForm::begin([
                'method' => 'get',
                'action' => Url::to(['submission/submission-report-duration', 'staff' => isset($staff) ? $staff : "", 'status' => isset($status) ? $status : "", 'submissionTypeGroupId' => isset($submissionTypeGroupId) ? $submissionTypeGroupId : ""]),
                'options' => [
                    'data-pjax' => 1,
                ],
