...
$this->title = Yii::t('app', 'สรุปประเภทและจำนวนโครงการวิจัยที่เสนอขอรับการพิจารณา');
...
<div class="site-about">
    <?php
    echo $this->renderFile('@app/views/site/_search-agenda-5year.php', ['searchModel' => $searchModel]);
    ?>
    <div class="panel">
        <?= Html::a(Yii::t('app', "EXPORT PDF"), ['site/agenda-5year-report', 'statusYear' => $searchModel->statusYear, 'pdf' => true], ['class' => 'btn btn-default pull-right btn-lg margin-10', 'type' => "submit", 'target' => '_blank']) ?>
                    ...
                    <table style="border-collapse: collapse; border: 1px solid black; width: 100%">
                        ...
                        <tbody>
                            ...
                            foreach ($agendas as $agenda):
                                ...
                                <tr>
                                    ...
                                    foreach ($filterYears as $filterYear):
                                        $query = app\models\Submission::find()->joinWith(['project'])->isDeleted(false)->hasMeetingAgendaPanelTotal($agenda->id)->yearStatus($filterYear, app\models\Submission::STATUS_AGENDA_ADDED);
                                        if (!empty($searchModel->panel_id)) {
                                            $query->panel($searchModel->panel_id);
                                        }
                                        $sm = $query->count();
                                        $total += $sm;
                                        ?>
                            ...
                            <tr style="background-color: #878787">
                                ...
                                $totalP = 0;
                                foreach ($filterYears as $filterYear):
                                    $query = app\models\Submission::find()->joinWith(['project'])->panel($searchModel->panel_id)->isDeleted(false)->hasMeetingAgenda()->yearStatus($filterYear, app\models\Submission::STATUS_AGENDA_ADDED);
                                    if (!empty($searchModel->panel_id)) {
                                        $query->panel($searchModel->panel_id);
                                    }
                                    $smp = $query->count();
