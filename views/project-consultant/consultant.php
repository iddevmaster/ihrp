...
//                'beforeGrid' => '<input type="hidden" name="step" value="2"',
            ...
            'toolbar' => [
                [
                    ...
                    'content' =>
                    (((($submission->submission_type_id == 9 && isset($submission->responsible_person)) || $submission->status < app\models\Submission::STATUS_MEETING_DONE || $submission->submissionType->submission_type_group_id == app\models\SubmissionTypeGroup::GROUP_NEW) && $currentRole['role_id'] == app\models\Role::STAFF) ? Html::a('<i class="glyphicon glyphicon-plus"></i> ' . Yii::t('app', 'เพิ่มอาจารย์ที่ปรึกษา'), ['project-consultant/create', 'submissionId' => $submission->id, 'staff' => $staff], ['role' => 'modal-remote', 'class' => 'btn btn-success btn-raised']) : "" ) .
                    Html::a('<i class="glyphicon glyphicon-repeat"></i>', Url::current(), ['data-pjax' => 1, 'class' => 'btn btn-default grey-600', 'title' => 'โหลดใหม่'])
                ],
            ],
            'columns' => require(__DIR__ . '/_columns-consultant.php'),
            'striped' => true,
            'condensed' => true,
            'responsive' => true,
            'summary' => false,
            'panel' => [
                // 'type' => 'primary',
                'heading' => Yii::t('app', 'รายชื่อที่ปรึกษาโครงการ'),
