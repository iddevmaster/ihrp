    ...
    $items[] = [
        ...
        'visibleButtons' => [
            'download' => function($model) use ($currentRole) {
                ...
                return ($currentRole['role_id'] == \app\models\Role::RESEARCHER || $currentRole['role_id'] == \app\models\Role::COORDINATOR) && isset($model->document->template_file) && $model->submission->status <= app\models\Submission::STATUS_SUBMITTED;
            },
            'upload' => function($model)use ($revise) {
                return (isset($model->submission->status) && ($model->submission->status < app\models\Submission::STATUS_WAITING_APPROVE_PROJECT_RESEARCHER && $model->submission->status > app\models\Submission::STATUS_NOT_APPROVE_PROJECT_RESEARCHER)) and ( isset($revise) || $model->status != SubmissionDocument::STATUS_PASS);
            },
            'delete' => function($model)use ($currentRole) {
                return isset($model->submission->status) && $model->submission->status < app\models\Submission::STATUS_SUBMITTED && isset($model->submission->project->projectLeader) && (($currentRole['role_id'] == \app\models\Role::RESEARCHER && $model->submission->project->projectLeader->person->id == \Yii::$app->user->identity->person->id) || ($currentRole['role_id'] == \app\models\Role::COORDINATOR && $model->submission->project_coordinator_id == \Yii::$app->user->id));
            }
        ]
    ];
}
